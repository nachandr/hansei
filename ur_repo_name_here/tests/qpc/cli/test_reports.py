...
def test_detail_report(
    ...
    for report_source, scan_source in zip(report['sources'], scan['sources']):
        ...
        for facts in report_source['facts']:
            report_facts = set(sorted(facts.keys()))
            expected_facts = set(sorted(FACTS))
            assert expected_facts.issubset(report_facts), (
                'Extra report facts:\n{}\n\nExtra expected facts:\n{}'.format(
                    pprint.pformat(report_facts - expected_facts),
                    pprint.pformat(expected_facts - report_facts),
                )
            )
            priviledged_facts = report_facts - expected_facts
            expected_priviledged_facts = set(sorted(EXTRA_FACTS))
            if len(priviledged_facts) > 0:
                # The user running scan has sudo privileged or is root
                assert expected_priviledged_facts.issubset(
                    priviledged_facts), (
                        'Extra report privileged facts:\n{}\n\nExtra expected '
                        'privileged facts:\n{}'.format(
                            pprint.pformat(
                                priviledged_facts - expected_priviledged_facts
                            ),
                            pprint.pformat(
                                expected_priviledged_facts - priviledged_facts
                            ),
                        )
                )
